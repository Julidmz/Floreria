...
<%! 
// Clase para representar una flor seleccionada
class Flor {
    String nombre;
    int cantidad;
    double precio;
    ...
    <div class="container my-4">
        ...
        <%
            ...
            if (idCliente != null) {
                ...
                try {
                    Class.forName("com.mysql.cj.jdbc.Driver");
                    conn2 = DriverManager.getConnection("jdbc:mysql://localhost:3306/floreria", "root", "");
        ...
        <form method="POST" action="">
            <div class="mb-3">
                <label for="zapato" class="form-label">Selecciona una Flor:</label>
                <select class="form-select" id="zapato" name="idZapato">
                    ...
                    <%
                        ...
                        try {
                            Class.forName("com.mysql.cj.jdbc.Driver");
                            conn3 = DriverManager.getConnection("jdbc:mysql://localhost:3306/floreria", "root", "");

                            String sql3 = "SELECT id, nombre, cantidad, precio FROM flor ORDER BY modelo ASC";
                    ...
                    %>
                                <option value="<%= id %>"><%= modelo + " (Nombre: " + nombre + ", Precio Venta: $" + precioVenta  + ")" %></option>
                    <%
                            }
                        } catch (Exception e) {
                            e.printStackTrace();
            ...
            <button type="submit" class="btn btn-secondary">Agregar Flor</button>
        </form>

        <!-- Lista de zapatos seleccionados -->
        <%
            if (request.getMethod().equalsIgnoreCase("POST")) {
                String idZapato = request.getParameter("idFlor");
                if (idFlor != null) {
                    ...
                    try {
                        Class.forName("com.mysql.cj.jdbc.Driver");
                        connFlor = DriverManager.getConnection("jdbc:mysql://localhost:3306/floreria", "root", "");

                        String sqlFlor = "SELECT id,nombre,cantidad,precio FROM flor WHERE id = ?";
                        ...
                        if (rsFlor.next()) {
                            ...
                            floresSeleccionadas.add(new Flor(nombre,cantidad,precio));
                        }
                    } catch (Exception e) {
                        e.printStackTrace();
                    } finally {
